...
library: "platform"
created: "2026-07-13T07:59:13.716429+00:00"
---
...
`max_tokens`:`max_tokens` is a hard per-request cap on generated tokens, while `task_budget` is an advisory cap across the full agentic loop (potentially spanning many requests). At `xhigh` or `max` effort, set `max_tokens` to at least 64k to give Claude room to think and act on each request.`task_budget.remaining` on each follow-up request, the changed value invalidates any cache prefix that contains it. To preserve caching, set the budget once on the initial request and let the model self-regulate against the server-side countdown rather than mutating the budget client-side.| Model | Support | 
|---|---|
...
Task budgets are not supported on Claude Code or Cowork surfaces. Use task budgets directly through the Messages API on a supported model.
...
- Source: [https://platform.claude.com/docs/en/build-with-claude/task-budgets](https://platform.claude.com/docs/en/build-with-claude/task-budgets)
